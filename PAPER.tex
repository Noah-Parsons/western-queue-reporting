...
\title{Who Reports Withdrawal Dates in the Western\\Interconnection Queue, and Does It Matter?}
\author{Noah Parsons}
...
service type all differ from the regional population at $p < 10^{-11}$, while
...
companion logistic regression over all \num{8097} western requests, requiring no
dates, finds solar the most withdrawal-prone technology, larger projects likelier
...
that carry a recorded withdrawal date. In the 2026 Edition data file, that is 637
...
population at $p < 10^{-11}$ after correction for multiple comparisons, on both
...
Southeast, and ``West''---the non-ISO Western Interconnection, i.e.\ WECC
excluding CAISO. That western region is the study population:
...
ground. By this definition 637 of the West's \num{5277} withdrawals are dated;
one of those lacks a queue year and so drops out of the cohort-stratified
analysis, leaving 636.
...
\textbf{West} & \textbf{\num{5277}} & \textbf{637} & \textbf{\SI{12.1}{\percent}} \\
\bottomrule
\end{tabular}
\end{table}
...
as a share of the provider's withdrawals. Reporting providers in bold.}
\begin{tabular}{lrrrr}
\toprule
...
The remaining nine providers have fewer than 25 requests each and none records a
date.
...
provider. Whatever the typical PacifiCorp withdrawal duration is, it contributes
\SI{6}{\percent} of its weight to the published median.
...
provider---obtainable, in principle, from FERC Form 715 filings or direct queue
archives, but not from this data file.
...
\SI{31}{\percent} of the region's withdrawals and \SI{6}{\percent} of its
recorded withdrawal dates.
...
differ at $p < 10^{-11}$; only project capacity does not.
